...
function Financeiro() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setNovaDespesa({ descricao: '', numero_documento: '', valor: '', data_vencimento: '', tipo: 'FIXA', situacao: 'PENDENTE' });
      carregarDespesas();
    } catch (error) {
            ...
            <table style={{width:'100%', borderCollapse:'collapse', minWidth: '600px'}}>
                <thead style={{background:'#f9fafb', borderBottom:'1px solid #e5e7eb'}}>
                    <tr>
                        ...
                        <th style={{padding:'15px', textAlign:'left', fontSize:'12px', color:'#6b7280'}}>DOC</th>
                    ...
                    {despesasFiltradas.length > 0 ? (
                        despesasFiltradas.map(d => (
                            <tr key={d.id} style={{borderBottom:'1px solid #f3f4f6'}}>
                                ...
                                <td style={{padding:'15px', fontSize:'12px', color:'#6b7280'}}>{d.numero_documento || '-'}</td>
