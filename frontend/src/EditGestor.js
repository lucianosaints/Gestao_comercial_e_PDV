...
import { FaUserPlus, FaSave, FaArrowLeft, FaStore, FaIdCard, FaEnvelope, FaUser, FaLock, FaUserTag, FaEdit } from 'react-icons/fa';
...
function EditGestor() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setListaUnidades(resUnidades.data);

                // Carregar Gestor
                const resGestor = await axios.get(`${API_BASE_URL}/api/gestores/${id}/`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                const gestor = resGestor.data;

                setFormData({
                    nome: gestor.nome,
                    cpf: gestor.cpf,
                    email: gestor.email,
                    password: '', // Senha vazia por padrão para não alterar
                    unidade: gestor.unidade || '',
                    cargo: gestor.cargo || 'vendedor'
                });
                setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        // Se a senha estiver vazia, remove ela do envio para o backend não alterar (backend deve tratar isso)
        // Se o backend espera a senha sempre, teremos que ver. Mas geralmente o viewset do Django ignora se não enviar, ou precisamos tratar.
        // O ModelViewSet padrão do DRF tentará atualizar tudo.
        // Vamos enviar tudo, mas se a senha for vazia, o backend precisa saber lidar. 
        // Como estamos usando o serializer padrão, ele pode exigir a senha se for required.
        // Verificaremos o serializer depois. Mas vamos tentar enviar assim.
        ...
        try {
            await axios.patch(`${API_BASE_URL}/api/gestores/${id}/`, dadosParaEnviar, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
        ...
        <div style={s.container}>
            ...
            <main style={s.main}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', maxWidth: '600px', margin: '0 auto 25px auto' }}>
                    <h1 style={{ margin: 0, color: '#1f2937', display: 'flex', alignItems: 'center', gap: '10px' }}><FaEdit style={{ color: '#eab308' }} /> Editar Usuário</h1>
                ...
                <form onSubmit={handleSubmit} style={s.card}>
                    ...
                    <div style={s.inputGroup}><label style={s.label}>Nova Senha (Opcional)</label><div style={s.inputWrapper}><FaLock style={s.iconInput} /><input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Deixe em branco para manter a atual" style={s.input} /></div><small style={{ color: '#6b7280', fontSize: '12px', marginTop: '5px', display: 'block' }}>Preencha apenas se quiser alterar a senha.</small></div>

                    <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'flex-end' }}><button type="submit" style={{ background: '#2563eb', color: 'white', border: 'none', padding: '12px 25px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}><FaSave /> Salvar Alterações</button></div>
                </form>
            </main>
        </div>
    );
}
export default EditGestor;
