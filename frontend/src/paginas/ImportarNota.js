...
function ImportarNota() {
    ...
    const enviarArquivo = async (file) => {
        ...
        try {
            ...
            const response = await axios.post('http://127.0.0.1:8000/api/importar-xml/', formData, {
                headers: {
    ...
    const confirmarImportacao = async () => {
        ...
        try {
            const token = localStorage.getItem('access_token');
            const response = await axios.post('http://127.0.0.1:8000/api/importar-xml/', previewData, {
